...
public:
    FileBrowserXGui()
    {
        initializePin(pinFilePath, static_cast<MpGuiBaseMemberPtr2>(&FileBrowserXGui::onSetPath));
    ...
    void onSetChoice()
    {
        ...
        if (choiceIndex >= 0 && choiceIndex < static_cast<int>(files.size()))
        {
            ...
            std::string fullPath = directoryPath + separator + wstring_to_utf8(filename) + targetExt;
...
private:
    ...
    namespace
    {
        auto r = Register<FileBrowserXGui>::withId(L"FileBrowserX");
    }
