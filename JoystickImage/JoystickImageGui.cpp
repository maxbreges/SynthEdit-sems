...
GMPI_REGISTER_GUI(MP_SUB_TYPE_GUI2, JoystickImageGui, L"JoystickImage");
...
{	
		...
		{
			pinPositionX = pinResetValue; // Reset the color adjustment flag
			pinPositionY = pinResetValue;
		}
	...
	if (!bitmap_.isNull())
	{
		...
		if (pinJumpToMouse && ((flags & gmpi_gui_api::GG_POINTER_KEY_CONTROL) == 0))
		{
			auto r = getRect();
