...
public:
		...
		)
	{
		...
		initializePin(pinTrig);
		initializePin(pinVel);
	}
	...
	{
		auto outputVel = getBuffer(pinVel);

		for (int s = sampleFrames; s > 0; --s)
		{
			*outputVel++ = vel;

			pinTrig.setValue(false, getBlockPosition() +s);
		}
	}
...
namespace
{
	auto r = Register<MidiLearn>::withId(L"MidiLearnX");
}
