...
class MIDIToInt final : public MpBase2
{

	MidiInPin pinMIDIIn;
	FloatOutPin pinChan;
	FloatOutPin pinNote;
	FloatOutPin pinVelocity;
...
public:		
		...
		void onMidi2Message(const midi::message_view& msg)
		{
			...
			pinChan = static_cast<uint8_t>(msg[1] & 0x0f) + 1;
			...
			case gmpi::midi_2_0::NoteOn:
			{
				...
				pinVelocity = (int)(float)(127.f * note.velocity);
				pinStatus = 1.0f;
			}
			break;
...
namespace
{
	auto r = Register<MIDIToInt>::withId(L"MIDIToFloatGui");
}
