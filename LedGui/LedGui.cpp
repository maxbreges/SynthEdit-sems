...
public:
	LedCircleGui()
	{
		...
		initializePin(pinBool, static_cast<MpGuiBaseMemberPtr2>(&LedCircleGui::onSetColor));
	...
	void calcDimensionsBg(Point& centerBg, float& radiusBg, float& thicknessBg)
	{
		...
		radiusBg = (std::min)(rBg.getWidth(), rBg.getHeight()) * 0.4f;
...
namespace
{
	auto r = Register<LedCircleGui>::withId(L"LedCircle");
}
